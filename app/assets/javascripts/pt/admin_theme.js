...
var App = function () {
  ...
  var sidebarWidth = '250px'; //* BEGIN:CORE HANDLERS *//
  ...
  var handleResponsive = function () {
    ...
    $('.toggle-sidebar').click(function (e) {
      e.preventDefault(); // Reset manual divider-resize
      $('#sidebar').css('width', '');
      $('#sidebar > #divider').css('margin-left', '');
      $('#content').css('margin-left', ''); // Toggle class
    ...
    var handleElements = function () {
      ...
      $('.crumbs .crumb-buttons > li:visible:first').addClass('first'); // Remove phone-navigation
      if ($('body').hasClass('nav-open')) {
        $('body').toggleClass('nav-open');
      }
      // Add additional scrollbars
      handleScrollbars(); // Handle project switcher width
  ...
  var handleLayout = function () {
    calculateHeight(); // For margin to top, if header is fixed
  ...
  var _resizeEvents = function () {
    calculateHeight(); // Realign headers from DataTables (otherwise header will have an offset)
    ...
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) && android_chrome == false) {
      ...
      if ($('#sidebar').hasClass('sidebar-fixed') || $(window).width() <= 767) {
        ...
        if (android_chrome) {
          ...
          $('#sidebar').attr('style', 'position: absolute !important;'); // Fix for really high tablet resolutions
          if ($(window).width() > 979) {
  ...
  var handleThemeSwitcher = function () {
    ...
    function _changeTheme(theme) {
      ...
      $('body').addClass('theme-' + theme); // Store it for page refresh
      $.cookie('theme', theme, {
        path: '/'
      }); // Button styles
      if (theme == 'dark') {
    ...
    if ($.cookie) {
      ...
      if ($.cookie('theme')) {
        ...
        _changeTheme(cookie_theme); // To select the right switch
  ...
  var handleTabs = function () {
    ...
    fixTabHeight('.nav.nav-tabs.tabs-left > li.active > a[data-toggle="tab"], .nav.nav-tabs.tabs-right > li.active > a[data-toggle="tab"]'); // activate tab if tab id provided in the URL
    if (location.hash) {
  ...
  var handleProjectSwitcher = function () {
    ...
    $('.project-switcher-btn').click(function (e) {
      ...
      $(this).parent().toggleClass('open'); // Define default project switcher
    ...
    $('body').click(function (e) {
      ...
      if ($.inArray('project-switcher', classes) == -1 && $.inArray('project-switcher-btn', classes) == -1 && $(e.target).parents().index($('.project-switcher')) == -1 && $(e.target).parents('.project-switcher-btn').length == 0) {
    ...
    var _hideVisibleProjectSwitcher = function (el) {
      $('.project-switcher').each(function () {
        var $projectswitcher = $(this); // Only slide up visible project switcher
        if ($projectswitcher.is(':visible')) {
          ...
          if (data_projectSwitcher != ('#' + $projectswitcher.attr('id'))) {
            $(this).slideUp(200, function () {
              $(this).toggleClass('open'); // Remove all clicked states from toggle buttons
  ...
  var handleProjectSwitcherWidth = function () {
    $('.project-switcher').each(function () {
      ...
      $projectswitcher.css('position', 'absolute').css('margin-top', '-1000px').show(); // Iterate through each li
      var total_width = 0;
